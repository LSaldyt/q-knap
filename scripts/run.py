...
from .util import basename, timedblock, verify_set, to_set, get_iterations
...
def interpret_output(output):
    ...
    for name, bit in namedBits:
        if '[' in name:
            ...
            d[name] = bit
    result = list(d.items())
    comp = lambda item : '$' not in item[0] and item[0] != 'Name(s)' and 'valid' not in item[0]
    return [item for item in result if comp(item)]

def to_output(filename, iterations=1):
    ...
    runc('qmasm --format=qbsolv -o output/qubos/@.qubo --pin="@.valid:= true" output/qmasms/@.qmasm', filename)
